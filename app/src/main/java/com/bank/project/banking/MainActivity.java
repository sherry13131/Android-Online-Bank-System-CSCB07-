...
import com.bank.project.banking.admin.AdminMenuActivity;
import com.bank.project.banking.atm.AtmMenuActivity;
import com.bank.project.banking.teller.TellerMenuActivity;
...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (accountTypes.size() == 0) {
      ...
      db.insertAccountType(AccountTypes.TFSA.toString(), new BigDecimal(0.25));
        ...
        if (InputValidator.isEmpty(userIdView) || InputValidator.isEmpty(passwordView)) {
          ...
          if (auth) {
            ...
            if (userRoleId == -1) {
              ...
              intent = new Intent(MainActivity.this, TellerMenuActivity.class);
              intent.putExtra("tellerId", userId);
              startActivity(intent);

            } else if (userRoleId == roles.get(Roles.CUSTOMER)) {
              // user is a customer so go to atm
              intent = new Intent(MainActivity.this, AtmMenuActivity.class);
